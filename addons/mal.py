...
class Mal:
    ...
    async def mal(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await self.bot.say('Invalid Syntax. See `>help mal` for more info on how to use this command.')
    ...
    async def anime(self, ctx, *, msg: str = None):
        ...
        if msg:
            fetch = await self.bot.say('Searching...')
            ...
            if not embed_perms(ctx.message) or link is True:
                await self.bot.say('https://myanimelist.net/anime/%s' % results.id)
                await fetch.delete()
                return await ctx.message.delete()
            ...
            await self.bot.say('Specify an anime to search for.')
    ...
    async def manga(self, ctx, *, msg: str = None):
        ...
        if msg:
            ...
            if not embed_perms(ctx.message) or link is True:
                await self.bot.say('https://myanimelist.net/manga/%s' % results.id)
                await fetch.delete()
                return await ctx.message.delete()
    ...
    async def va(self, ctx, *, query):
        ...
        va_info = await self.bot.say(content=None, embed=em)
        await fetch.delete()
        await ctx.message.delete()
    ...
    async def next_(self, ctx, *, query):
        ...
        search = await self.bot.say("Searching...")
        ...
        if found:
            ...
            await self.bot.say('Failed to find given anime.')
            await search.delete()
            return await ctx.message.delete()
